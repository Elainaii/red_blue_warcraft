...
#include "sstream"
headquarter::headquarter(int sp,int city,int cityNum, const std::string& color, std::vector<warriorType> produceOrder,std::vector<std::vector<warrior*>> & cityList,EventControl& eventManager)
: sp(sp), color(color), produceOrder(produceOrder),roll(0),city(city),cityNum(cityNum),cityList(cityList),eventManager(eventManager)
{
	...
	while (time--) {//循环5次
		...
		case DRAGON:
			if(sp>=dragon::preHp){
				...
				//产生事件字符串
				oss<<color<<" dragon "<<id<<" born with strength "<<dragon::preHp<<","<<warriorNum[DRAGON]<<" dragon in "<<color<<" headquarter"<<std::endl;
				oss<<"It has a "<<temp->getWeapon(0).getType()<<" and it's morale is "<<std::fixed<<std::setprecision(2)<<temp->getMorale()<<std::endl;
		...
		case NINJA:
			if(sp>=ninja::preHp){
				...
				oss<<color<<" ninja "<<id<<" born with strength "<<ninja::preHp<<","<<warriorNum[NINJA]<<" ninja in "<<color<<" headquarter"<<std::endl;
				oss<<"It has a "<<temp->getWeapon(0).getType()<<" and a "<<temp->getWeapon(1).getType()<<std::endl;
				Event eve(Time,city,colorFlag,BORN,oss.str());
				eventManager.addEvent(eve);
			}
			break;
...
//				std::cout<<"It has a "<<temp->getWeapon(0).getType()<<std::endl;
				oss<<color<<" iceman "<<id<<" born with strength "<<iceman::preHp<<","<<warriorNum[ICEMAN]<<" iceman in "<<color<<" headquarter"<<std::endl;
...
//				std::cout<<"It's loyalty is "<<sp<<std::endl;
				oss<<color<<" lion "<<id<<" born with strength "<<lion::preHp<<","<<warriorNum[LION]<<" lion in "<<color<<" headquarter"<<std::endl;
				oss<<"It's loyalty is "<<sp<<std::endl;
				Event eve(Time,city,colorFlag,BORN,oss.str());
				eventManager.addEvent(eve);
			}
			break;
...
//				std::cout<<color<<" wolf "<<id<<" born with strength "<<wolf::preHp<<","<<warriorNum[WOLF]<<" wolf in "<<color<<" headquarter"<<std::endl;
				oss<<color<<" wolf "<<id<<" born with strength "<<wolf::preHp<<","<<warriorNum[WOLF]<<" wolf in "<<color<<" headquarter"<<std::endl;
				Event eve(Time,city,colorFlag,BORN,oss.str());
				eventManager.addEvent(eve);
			}
			break;
...
void headquarter::reportWeapon(int Time) const {//接收每个武士报告的字符串并打包成事件添加
	for(auto i:warriorList){
		Event eve(Time,city,colorFlag,REPORT,i->reportWeapon());
		eventManager.addEvent(eve);
	}
}
...
void headquarter::reportSp(int Time) const {
	...
	Event eve(Time,city,colorFlag,REPORT,oss.str());
	eventManager.addEvent(eve);
}
bool headquarter::warriorMove(int Time) {//移动到了敌方司令部返回true
	...
	for(int i =0;i<warriorList.size();i++){//武士序号在前面的肯定走在前面
		if(colorFlag==0){//红队向右走
			...
			oss<<color<<" "<<warriorList[i]->getName()<<" "<<warriorList[i]->getId()<<" marched to city "<<warriorList[i]->getCity()<<" with "<<warriorList[i]->getHp()<<" elements and force "<<warriorList[i]->getAtk()<<std::endl;
			Event eve(Time,warriorList[i]->getCity(),colorFlag,MOVE,oss.str());
			eventManager.addEvent(eve);
			if(warriorList[i]->getCity()==cityNum+1){//到达敌方司令部
				oss.str("");
				oss<<"red "<<warriorList[i]->getName()<<" "<<warriorList[i]->getId()<<" reached blue headquarter with "<<warriorList[i]->getHp()<<" elements remaining"<<std::endl;
				Event eve(Time,city,colorFlag,REACH,oss.str());
				eventManager.addEvent(eve);
				flag = true;
			}
		}
			...
			if(warriorList[i]->getCity()==cityNum+1){//到达敌方司令部
				oss.str("");
				oss<<"blue "<<warriorList[i]->getName()<<" "<<warriorList[i]->getId()<<" reached red headquarter with "<<warriorList[i]->getHp()<<" elements remaining"<<std::endl;
				Event eve(Time,city,colorFlag,REACH,oss.str());
				eventManager.addEvent(eve);
				flag = true;
			}
	...
	for (int i = 1; i < cityNum + 1; i++) {
		if (cityList[i][0] && cityList[i][1]) {//这个城市有两个武士
			if(cityList[i][0]->getType()==cityList[i][1]->getType()){
				...
				std::string str;
				if(!(str=temp->rob(*cityList[i][1])).empty()){
					std::ostringstream oss;
					oss<<"red wolf "<<cityList[i][0]->getId()<<" took "<<str<<" from blue "<<cityList[i][1]->getName()<<" "<<cityList[i][1]->getId()<<" in city "<<i<<std::endl;
				...
				if(!(str=temp->rob(*cityList[i][0])).empty()){
					std::ostringstream oss;
					oss<<"blue wolf "<<cityList[i][1]->getId()<<" took "<<str<<" from red "<<cityList[i][0]->getName()<<" "<<cityList[i][0]->getId()<<" in city "<<i<<std::endl;
